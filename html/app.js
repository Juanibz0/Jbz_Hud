...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				else if (config.showServerLogo == false) { document.querySelector('.info.player').style.display = 'block'; }
			...
			updateVehicle: function(data) {
				...
				if (data.status == true) {
					...
					if ( (data.seatbelt.status == true) && (vehicleSeatbelt.classList.contains('on') == false) ) {
						...
						eventCallback.sound('sounds/seatbelt-buckle.ogg', { volume: '0.80' });
					}
						...
						eventCallback.sound('sounds/seatbelt-unbuckle.ogg', { volume: '0.80' });
					}
