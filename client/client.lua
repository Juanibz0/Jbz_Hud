...
Citizen.CreateThread(function()
	...
	while true do
		...
		TriggerServerEvent('Jbz_Hud:getServerInfo')
...
-- Overall Info
RegisterNetEvent('Jbz_Hud:setInfo')
AddEventHandler('Jbz_Hud:setInfo', function(info)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and (has_value(vehiclesCars, vehicleClass) == true) then
			...
			TriggerEvent('Jbz_Hud:setCarSignalLights', vehicleSignalIndicator)
		end
...
AddEventHandler('Jbz_Hud:setCarSignalLights', function(status)
	...
	TriggerServerEvent('Jbz_Hud:syncCarLights', status)
...
RegisterNetEvent('Jbz_Hud:syncCarLights')
AddEventHandler('Jbz_Hud:syncCarLights', function(driver, status)
